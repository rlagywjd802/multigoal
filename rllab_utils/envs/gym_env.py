...
from gym_blocks.wrappers.monitoring import Monitor as MonitorBlocks
...
class GymEnv(Env, Serializable):
    def __init__(self, env_name, record_video=True, video_schedule=None, log_dir=None, record_log=True,
        ...
        if log_dir is None or record_log is False:
            ...
            if not record_video:
                print('!!! GymEnv: NoVideoSchedule assigned. Video will not be recorded !!!')
            ...
            if env.spec.id[:6] == 'Blocks' and env.spec.id[:12] != 'BlocksSimple':
                print('!!! GymEnv: Special Monitor wrapper for Blocks env will be used')
                self.env = MonitorBlocks(self.env, log_dir  + '/gym_log', video_callable=video_schedule, force=True)
            else:
