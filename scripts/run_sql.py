...
import rllab_utils.policies.gaussian_multiobs_policy as gaus_pol
from rllab_utils.baselines.baselines import GaussianConvBaseline
...
for param_tuple in param_cart_product:
    ...
    pool = SimpleReplayBuffer(
        env=env, max_replay_buffer_size=params['max_replay_buffer_size'])

    sampler = SimpleSampler(
        max_path_length=params['timelen_max'],
        min_pool_size=params['max_replay_buffer_size']*3/4,
        batch_size=params['batch_size'])

    base_kwargs = dict(
        epoch_length=params['timelen_max'],
        n_epochs=params['iterations'],
        n_train_repeat=1,
        eval_render=False,
        eval_n_episodes=params['test_episodes_num'],
        sampler=sampler,
        ## TRPO
        batch_size=params['batch_size'],
    ...
    M = [32, 32]#params['seek_fuse_net_params']['hidden_sizes']
    qf = NNQFunction(env=env, hidden_layer_sizes=(M[0], M[1]))

    seek_policy = StochasticNNPolicy(env=env, hidden_layer_sizes=(M[0], M[1]))    ##### size 맞추기
    ...
    algo = SQL(
        ...
        kernel_n_particles=params['kernel_particles'],
        kernel_update_ratio=params['kernel_update_ratio'],
        value_n_particles=params['value_n_particles'],
        td_target_update_interval=params['td_target_update_interval'],
        qf_lr=params['qf_lr'],
